...
class CustomLogger:
    ...
    def log_around(wrapped, instance, args, kwargs):
        ...
        # Check if 'self' is present in the method signature
        signature = inspect.signature(wrapped)
        if 'self' not in signature.parameters:
            warning_message = f"Warning: Method '{class_name}.{method_name}' does not contain 'self' parameter."
            CustomLogger.get_instance().log_aspect('WARNING', warning_message)

            entry_message = f"Entering method '{class_name}.{method_name}' without 'self' with level '{log_level}': {message}"
